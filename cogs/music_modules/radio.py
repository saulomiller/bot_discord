...
class MusicRadioMixin:
    ...
    async def _do_list_radios(self, ctx_or_interaction):
        ...
        for radio in radios:
            ...
            embed.add_field(
                name=f"{name} ({radio_id})", value=location, inline=True
            )
    ...
    @app_commands.command(
        name="radios", description="Lists available radio stations"
    )
    ...
    async def _do_play_radio(self, ctx_or_interaction, radio_id: str):
        ...
        try:
            ...
            # Evita que o callback de loop re-enfileire a música anterior.
            player.current_song = None

            # Se já estiver tocando/pausado, stop dispara o after_play.
            # O callback chama play_next e inicia a rádio recém-enfileirada.
    ...
    @app_commands.command(
        name="radio", description="Plays a specific radio station"
    )
    @app_commands.describe(radio_id="Radio ID")
    async def radio_slash(
        self, interaction: discord.Interaction, radio_id: str
    ):
        """Executa o comando slash de radio."""
        await self._do_play_radio(interaction, radio_id)

    async def _do_add_radio(
        self,
        ctx_or_interaction,
        id: str,
        nome: str,
        ...
        if not RADIO_ID_RE.fullmatch(radio_id):
            embed = EmbedBuilder.create_error_embed(
                t("error"),
                "ID invalido. Use apenas letras, numeros, '_' e '-', "
                "ate 64 caracteres.",
            )
            if isinstance(ctx_or_interaction, discord.Interaction):
                await ctx_or_interaction.response.send_message(
        ...
        if self._find_radio(radio_id):
            embed = EmbedBuilder.create_error_embed(
                t("error"), t("radio_exists", name=nome)
            )
            if isinstance(ctx_or_interaction, discord.Interaction):
                await ctx_or_interaction.response.send_message(
                    embed=embed, ephemeral=True
        ...
        try:
            ...
            embed = EmbedBuilder.create_success_embed(
                t("radio_added"),
                t("radio_added_success", name=nome, id=radio_id),
            )
            if isinstance(ctx_or_interaction, discord.Interaction):
                await ctx_or_interaction.response.send_message(embed=embed)
            else:
    ...
    @app_commands.command(
        name="addradio", description="Adds a new custom radio (Admin)"
    )
    @app_commands.describe(
        id="Unique ID for remote",
        nome="Radio Name",
        url="Stream URL",
        localizacao="Location (optional)",
    )
    ...
    async def _do_remove_radio(self, ctx_or_interaction, id: str):
        ...
        try:
            ...
            embed = EmbedBuilder.create_success_embed(
                t("radio_removed"), t("radio_removed_success", name=name)
            )
            if isinstance(ctx_or_interaction, discord.Interaction):
                await ctx_or_interaction.response.send_message(embed=embed)
            else:
            ...
            embed_err = EmbedBuilder.create_error_embed(
                t("error"), t("error_removing_radio")
            )
            if isinstance(ctx_or_interaction, discord.Interaction):
                await ctx_or_interaction.response.send_message(
                    embed=embed_err, ephemeral=True
    ...
    @app_commands.command(
        name="removeradio", description="Removes an existing radio (Admin)"
    )
    @app_commands.describe(id="ID of the radio to remove")
    async def remove_radio_slash(
