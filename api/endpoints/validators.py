...
def validate_upload_filename(
    ...
    *,
    ...
    if ext not in allowed_extensions:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Formato não suportado. Permitidos: {allowed}",
        )

    return safe_name


def ensure_matching_resource_id(
    path_id: str, body_id: str, *, label: str
) -> None:
    """Garante matching resource id."""
    if path_id != body_id:
        raise HTTPException(
            status_code=400, detail=f"{label} do caminho difere do payload."
        )
