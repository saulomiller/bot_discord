...
class QueueMixin:
    ...
    async def add_to_queue(self, search: str, user: discord.Member):
        ...
        try:
            ...
            # Limita a 1 entrada para evitar processar playlists inteiras.
            ...
            info = info_list[0]
            # Garante duration_seconds disponível na tupla retornada.
            ...
            if stream_url and self._is_direct_stream_url(stream_url):
                try:
                    await self.loop.run_in_executor(
                        None,
                        lambda: self._probe_stream_url(
                            stream_url, http_headers
                        ),
                    )
                except Exception as exc:
                    logging.warning(
                        "[add_to_queue] URL preventiva recusada; será "
                        "resolvida novamente no play: %s",
                        exc,
                    )
                else:
                    ...
                    logging.info(
                        "[add_to_queue] Stream URL validada e cacheada para: "
                        f"{song['title']}"
                    )
    ...
    async def _process_remaining_playlist(
        ...
        try:
            logging.info(
                "🎵 Processando restante da playlist (iniciando em %s)...",
                start_index,
            )
            ...
            for idx, entry in enumerate(valid_entries):
                ...
                if (
                    ...
                    logging.info(
                        "✓ Iniciando reprodução tardia via background: "
                        f"{song['title']}"
                    )
            ...
            logging.info(
                f"✓ Playlist completa: {added_count} músicas adicionadas "
                "à fila"
            )
