...
class ExtractionMixin:
    ...
    async def extract_info(self, search, max_entries=None):
        ...
        Returns:
            Lista de tuplas ``(title, url, thumbnail, duration, channel,
            duration_seconds, stream_url, http_headers)``.

        """

        def run() -> list:
            # Reusa self.ydl em vez de criar nova instância.
            info = None
            entries = []
            ...
            for entry in entries:
                ...
                try:
                    ...
                    # Prioriza URL canônica para evitar stream efêmero.
                    webpage_url = entry.get("webpage_url") or entry.get(
                        "original_url"
                    )
                    ...
                    if url and not url.startswith("http"):
                        ...
                        if ie_key == "youtube" or "youtube" in extractor:
                            ...
                            ie_key == "soundcloud" or "soundcloud" in extractor
                        ):
                            url = (
                    ...
                    # Se ainda vier URL direta de stream, usa a canônica.
                    ...
                    logging.warning(
                        f"Erro ao processar entrada da playlist: {e}"
                    )
                    continue
