...
class SoundboardMixin:
    ...
    async def play_soundboard(self, sfx_path: str, volume: float = 1.0):
        """Toca efeito do soundboard e retoma a música quando necessário."""
        if self.sfx_playing:
            return
        ...
        if self.voice_client.is_playing() and self.current_song:
            ...
            # Para o playback atual; o callback ignora stopped_for_sfx.
            self.voice_client.stop()

            # Aguarda o player realmente parar com polling curto.
        ...
        if is_opus_sfx and abs(volume - 1.0) < 0.01:
            ...
            logging.info(
                f"[sfx] mode=encode_opus | volume={volume} | path={sfx_path}"
            )
        ...
        def after_sfx(error):
            ...
            try:
                ...
                if self.stopped_for_sfx:
                    ...
                    # Retoma a música sem bloquear a thread do FFmpeg.
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    "Fallback para PCM no Soundboard devido a falha Opus: "
                    f"{opus_err}"
                )
