            ...
            if channel:
                ...
                while attempts < max_attempts:
                    try:
                        ...
                        attempts += 1
                        logging.warning(
                            f"Falha ao enviar dashboard (tentativa {attempts}/{max_attempts}): {e}"
                        )
                        # Se Forbidden, não adianta tentar de novo
                        if isinstance(e, discord.Forbidden):
                            logging.error(
                                "Sem permissão para enviar o dashboard no canal."
                            )
                            break
                ...
                if attempts >= max_attempts:
                    logging.error(
                        "Não foi possível enviar o dashboard após várias tentativas."
                    )
