...
class EmbedBuilder:
    ...
    @staticmethod
    def create_error_embed(title: str, description: str = "", suggestion: str = "") -> discord.Embed:
    ...
    @staticmethod
    def create_queue_embed(current_song: dict | None, queue_list: list) -> discord.Embed:
        """Embed compacto para fila (efêmero)."""
        total_songs = len(queue_list)

        # Duração total: suporta tanto int/float (segundos) quanto string "MM:SS"
        total_seconds = 0
        for song in queue_list:
            ...
            if isinstance(dur, (int, float)):
                ...
                # Tenta converter "MM:SS" ou "HH:MM:SS"; ignora 'Desconhecida' e '...'
                try:
        ...
        for i, song in enumerate(queue_list[:display_limit], 1):
            title    = song.get('title', t('unknown')) if isinstance(song, dict) else str(song)
            duration = song.get('duration', '?:??')   if isinstance(song, dict) else '?:??'
        ...
        return discord.Embed(description=description, color=EmbedBuilder.COLOR_INFO)
    ...
    def create_radio_embed(radio_info: dict) -> discord.Embed:
        ...
        return discord.Embed(description=embed_desc, color=EmbedBuilder.COLOR_RADIO)
