...
def normalize_radios_data(radios_data):
    ...
    def add_radio(raw_id, raw_radio, *, default_custom):
        """Adiciona uma rádio válida ao formato normalizado."""
...
def load_radios():
    ...
    if os.path.exists("radios.json"):
        try:
            logging.info(
                "Carregando radios.json da raiz e copiando para data/radios.json"
            )
...
async def extract_info(search: str) -> tuple[str, str, str, str, str]:
    ...
    def run():
        """Extrai metadados com yt-dlp para uso no executor."""
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                logging.info(
                    f"Informações extraídas com sucesso para: {title}"
                )
                return result
            except Exception as e:
                logging.error(
