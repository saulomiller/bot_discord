...
def sizeof_image(value):
    """Estima o tamanho de uma imagem ou blob para uso no cache."""
...
# Evita "value too large" limitando cada TTLCache a aproximadamente 50 MB.
MAX_CACHE_SIZE_BYTES = 50 * 1024 * 1024

image_cache_raw = TTLCache(
    maxsize=MAX_CACHE_SIZE_BYTES, ttl=300, getsizeof=sizeof_image
)
image_cache_processed = TTLCache(
    maxsize=MAX_CACHE_SIZE_BYTES, ttl=300, getsizeof=sizeof_image
)
    ...
    with _font_lock:
        if key not in FONTS:
            try:
                ...
                log.warning(
                    f"Fonte '{name}' não encontrada, usando padrão: {e}"
                )
...
@dataclass
class ImageAssets:
    """Agrupa imagens derivadas e cor dominante usadas no card."""
    ...
    def is_valid(self) -> bool:
        """Indica se os assets possuem a imagem original carregada."""
        return self.original is not None


def hash_bytes(data: bytes) -> str:
    """Gera um hash estável para bytes de imagem."""
...
def generate_thumbnail(img: Image.Image, size: Tuple[int, int]) -> Image.Image:
    """Gera thumbnail quadrada com crop central e downscale eficiente."""
    w, h = img.size
    min_side = min(w, h)
    left = (w - min_side) // 2
    top = (h - min_side) // 2
    ...
    if cropped.size != size:
        ...
        new_img.paste(
            cropped,
            (
                (size[0] - cropped.size[0]) // 2,
                (size[1] - cropped.size[1]) // 2,
            ),
        )
        return new_img
    return cropped
...
def get_dominant_color_from_bytes(content: bytes) -> tuple[int, int, int]:
    ...
    try:
        ...
        final_color = (
            rgb if isinstance(rgb, tuple) and len(rgb) == 3 else fallback_color
        )
...
def get_dominant_color(url: str) -> tuple[int, int, int]:
    """Baixa a imagem e devolve sua cor dominante."""
    try:
...
def truncate_text(
    ...
    """Trunca o texto para caber em uma largura máxima."""
...
def apply_side_gradient(base_img: Image.Image, start_x: int) -> None:
    """Aplica um gradiente escuro lateral sobre a imagem base."""
    ...
    base_rgba = (
        base_img if base_img.mode == "RGBA" else base_img.convert("RGBA")
    )
...
def get_vignette_layer(width: int, height: int) -> Image.Image:
    ...
    vignette_mask = vignette.point(
        lambda p: int(15 + 165 * ((255 - p) / 255.0))
    )
...
def draw_text_shadow(
    ...
    """Desenha texto com sombra simples para melhorar contraste."""
    x, y = pos
...
def wrap_two_lines(
    ...
    """Quebra o texto em até duas linhas respeitando a largura máxima."""
    words = text.split()
    lines = []
    current = ""
...
def normalize_song_info(song_info: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Normaliza metadados de música para o formato esperado pelo card."""
...
def prepare_image_assets(
    content: Optional[bytes], cover_size: int, width: int, height: int
) -> ImageAssets:
    ...
    try:
        ...
        with cache_lock:
            # Reconstrói via buffer comprimido para economizar memória.
            if (
                cache_key_thumb in image_cache_processed
                and cache_key_bg in image_cache_processed
            ):
                assets.thumbnail = Image.open(
                    BytesIO(image_cache_processed[cache_key_thumb])
                ).convert("RGB")
                assets.background = Image.open(
                    BytesIO(image_cache_processed[cache_key_bg])
                ).convert("RGB")
                return assets
        ...
        # Não clona o objeto; ``crop`` não o destrói neste fluxo.
...
def draw_card_layout(
    ...
    """ETAPA 3: Render desenha todos os elementos do card."""
    ...
    user_str = (
        str(user) if not hasattr(user, "display_name") else user.display_name
    )
    ...
    has_progress = progress_percent is not None and isinstance(
    ...
    if next_songs and isinstance(next_songs, list):
        for i, song in enumerate(next_songs[:max_lines], 1):
            s_title = (
                song.get("title", "?") if isinstance(song, dict) else str(song)
            )
            s_duration = (
    ...
    if has_progress:
        ...
        bar_x = panel_x + 14
        draw.rounded_rectangle(
            [bar_x, progress_y, bar_x + bar_w, progress_y + progress_h],
            radius=2,
            fill=(40, 40, 40),
        )
        # Foreground bar
        ...
        if fill_w > 0:
            draw.rounded_rectangle(
                [bar_x, progress_y, bar_x + fill_w, progress_y + progress_h],
                radius=2,
                fill=(29, 185, 84),
            )
