...
class AudioReactiveBackground {
  ...
  drawBars() {
    ...
    // Atualizar alturas das barras baseado em frequências
    for (let i = 0; i < barCount; i++) {
      const bar = this.bars[i];
      const index = Math.floor((i / barCount) * this.bufferLength);
      const value = this.dataArray[index] || 0;
      
      // Suavizar a transição de altura
      const targetHeight = (value / 255) * this.canvas.height * 0.5;
      bar.targetHeight = targetHeight;
      bar.height = bar.height * this.smoothing + targetHeight * (1 - this.smoothing);
    }
    ...
    for (let bar of this.bars) {
      ...
      gradient.addColorStop(0, `hsla(${bar.hue}, 100%, 55%, 0.8)`);
      gradient.addColorStop(0.5, `hsla(${bar.hue}, 100%, 50%, 0.9)`);
      gradient.addColorStop(1, `hsla(${bar.hue}, 100%, 45%, 1)`);
      
      this.ctx.fillStyle = gradient;
      this.ctx.fillRect(bar.x, centerY - bar.height, bar.width, bar.height);
    }

    // Desenhar reflexo (bottom half - invertido)
    for (let bar of this.bars) {
      const gradient = this.ctx.createLinearGradient(
        bar.x, centerY,
        bar.x, centerY + bar.height
      ...
      gradient.addColorStop(0, `hsla(${bar.hue}, 100%, 45%, 0.6)`);
      gradient.addColorStop(0.5, `hsla(${bar.hue}, 100%, 40%, 0.3)`);
      gradient.addColorStop(1, `hsla(${bar.hue}, 0%, 20%, 0)`);
...
async function updateStatus() {
  try {
    ...
    // Sincronizar animação de fundo com estado de play/pause
    liquidBg.syncPlayState(!isPaused && ready);
